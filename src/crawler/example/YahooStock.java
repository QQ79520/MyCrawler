 ...
 * 
 * 目標取回 Yahoo Stock指定個股的交易明細資料 但網頁只會保留最後50筆交易明細，所以必需排程 在交易日的 AM 09:00 - PM
 * 13:25不斷擷取 並在14:45再補上最後盤後交易記錄
 * 
 * 重點 1. 自動排程 2. 復習 mongodb
 * 
 * @author Abola Lee
 *
 */
public class YahooStock {
	...
	// 每次取得最後50筆交易的內容
	static String uri_format = "https://tw.stock.yahoo.com/q/ts?s=2498&t=50";

	public static void main(String[] args) {

		// 要有輸入參數 (run->run configurations->arguments)
		if (args.length >= 1) {
			stockNumber = args[0];
		} else {
			// 沒輸入參數
	...
	static void crawlerStockByNumber(String stockNumber) {
		...
		// 取得交易明細資料
		Elements transDetail = CrawlerPack.start().setRemoteEncoding("big5")// 設定遠端資料文件編碼
				.getFromHtml(uri)
				// 目標含有 成 交 明 細 的table
	...
	static List<DBObject> parseTransDetail(Elements transDetail) {
		...
		String day = transDetail.select("td:matchesOwn(資料日期)").text().substring(5, 14);

		// 取出 header 以外的所有交易資料
		// >>>Fill here<<<
		for (Element detail : transDetail.select("td>table>tbody>tr:gt(0)")) {
			Map<String, String> data = new HashMap<>();
			/*
			 * 資料格式範例 <tr align="center" bgcolor="#ffffff" height="25">
			 * <td>09:45:46</td> <td class="low">158.00</td> <td>158.50</td>
			 * <td>158.50</td> <td>－</td> <td>1</td> </tr>
			 */
	...
	static void saveTransDetail(List<DBObject> parsedTransDetail) {
		...
		try {
			...
			db.getCollection("QQ79520").insert(parsedTransDetail);
